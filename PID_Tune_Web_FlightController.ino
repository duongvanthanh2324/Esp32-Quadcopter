...
// Thông tin WiFi
//const char* ssid = "Happy Home 1"; 
//const char* password = "wifikhongcopass";
...
void gyro_signals(void) {
  ...
  // Chuyển đổi đơn vị đo sang độ/giây (65.5LSB = 1độ/giây)
  RateRoll = (float)GyroX / 65.5; 
  RatePitch = (float)GyroY / 65.5;
  RateYaw = (float)GyroZ / 65.5;
  //(4096 LSB/g <=> 1g = 4096LSB)
  AccX = (float)AccXLSB/4096;
  AccY = (float)AccYLSB/4096;
  AccZ = (float)AccZLSB/4096-0.22;

  AngleRoll = atan(AccY/sqrt(AccX*AccX+AccZ*AccZ))*57.296; //phép atan trả về radian nên đổi về độ (57.296=180/3.142)
...
void pid_equation(float Error,float P,float I,float D,float PrevError,float PrevIterm){
  ...
  if(Iterm > 400) Iterm=400;   //limit Iterm value -400, 400 to avoid the phenomena integral windup due to saturation
...
String processor(const String& var){
  ...
  return String(); // Trả về một chuỗi rỗng
}
...
//WIFI server setup END
  ...
  ledcAttach(esc_pin1, 250, 12); //ESC1
  ledcAttach(esc_pin2, 250, 12); //ESC2
  ledcAttach(esc_pin3, 250, 12); //ESC3
  ledcAttach(esc_pin4, 250, 12); //ESC4
  // Khởi động ESC ở mức thấp (1000us = min throttle)
  ledcWrite(esc_pin1, 1024);
  ledcWrite(esc_pin2, 1024);
  ledcWrite(esc_pin3, 1024);
  ledcWrite(esc_pin4, 1024);
  ...
  Wire.setClock(400000);    // Tốc độ giao tiếp I2C là 400kHz
  Wire.begin();             
  delay(250);               
  ...
  Wire.write(0x08);             
  Wire.endTransmission();      
  // for(RateCalibrationNumber=0; RateCalibrationNumber<2000; RateCalibrationNumber++){ //đo 2000 lần
  //   gyro_signals();
  ...
  //   delay(1);
  // }
  RateCalibrationRoll = 4.35; //chia lấy offset trung bình khi sensor đứng yên
  ...
  while (ReceiverValue[2] < 1020 || ReceiverValue[2] > 1050) {
    delay(10); // Đợi một chút để các ngắt cập nhật giá trị
    Serial.print("-"); // In dấu chấm để biết chương trình đang chờ
  }
  LoopTimer = micros();
}

void loop() {
  if(ReceiverValue[4] > 1500 ){ //channel 5 for uploading values, channel 5 gạt xuống
...
//  neutralPositionAdjustment();
  ...
  gyro_signals();                  // Đọc dữ liệu gyroscope

  RateRoll -= RateCalibrationRoll; //lấy giá trị đúng = giá trị đo được - độ lệch trung bình
  ...
  //Angle đang là độ phải đổi sang radian vì các phép lượng giác sin cos dùng radian
  ...
  AccZInertial=(AccZInertial-1)*9.81*100; //MPU không chỉ đo gia tốc chuyển động của vật thể mà nó còn đo cả gia tốc trọng trường(1G)
                                          //nên phải trừ đi gia tốc trọng trường để lấy gia tốc chuyển động của vật thể (cm/s²)
 
  barometer_signals();     // Đọc dữ liệu AltitudeBarometer
  ...
  if(ItermPitch > 400) ItermPitch=400;       // Limit Iterm value -400, 400 to avoid the phenomena integral windup due to saturation
  ...
  // Compute InputRoll, InputPitch, InputYaw
  ...
  // Limit throttle power at 80% (0%->1000us; 100%->2000us)
  if(InputThrottle>1800) InputThrottle=1800;

  // Propeller Configuration -> Prop out
  MotorInput1 = InputThrottle - InputPitch - InputRoll - InputYaw; // Motor 1 (Phía trước bên trái)
  MotorInput2 = InputThrottle + InputPitch - InputRoll + InputYaw; // Motor 2 (Phía sau bên trái)
  MotorInput3 = InputThrottle + InputPitch + InputRoll - InputYaw; // Motor 3 (Phía sau bên phải)
  MotorInput4 = InputThrottle - InputPitch + InputRoll + InputYaw; // Motor 4 (Phía trước bên phải)
  ...
  // Serial.print(MotorInput4); // End the line for Serial Plotter to process
  // Serial.println(" -- ");
